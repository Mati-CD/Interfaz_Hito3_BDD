...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _login() async {
    ...
    // Validación e inicio de sesión usando MockDatabase
    final user = await MockDatabase.login(rut);
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const Text('Sistema de Monitoreo CMG', style: TextStyle(fontSize: 16, color: Colors.grey)),
...
class DashboardScreen extends StatelessWidget {
  ...
  const DashboardScreen({super.key, required this.rutEncargado, required this.nombreEncargado});
          ...
          children: [
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Bienvenido, $nombreEncargado', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  Text('RUT: $rutEncargado', style: const TextStyle(fontSize: 14, color: Colors.grey)),
                  const SizedBox(height: 10),
                  const Divider(),
                  const SizedBox(height: 10),
                  const Text('Tus Barras Asignadas:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                ],
              ),
            ),
            const SizedBox(height: 10),
            Expanded(
              child: FutureBuilder<List<Map<String, dynamic>>>(
                future: MockDatabase.getBarrasByEncargado(rutEncargado),
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator());
                  }
                  if (!snapshot.hasData || snapshot.data!.isEmpty) {
                    return const Center(child: Text('No posees barras asignadas en el sistema.'));
                  }
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          title: Text(barra['nombre_barra'], style: const TextStyle(fontWeight: FontWeight.bold)),
                          subtitle: Text('Nivel de Tensión: ${barra['nivel_tension_kv']} kV'),
...
class BarraDetailScreen extends StatelessWidget {
  ...
  Widget _buildResumenAnualTab() {
    return FutureBuilder<Map<String, dynamic>>(
        future: MockDatabase.getResumenAnual(idBarra, 2025),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
          ...
          return Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Resumen Estadístico - Año 2025',
  ...
  Widget _buildComparativaTab() {
    return FutureBuilder<Map<String, dynamic>>(
      future: MockDatabase.getComparativaEstacional(idBarra),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
        ...
        return Center(
          child: Padding(
            ...
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                const Icon(Icons.compare_arrows_rounded, size: 64, color: Colors.blue),
                const SizedBox(height: 20),
                const Text(
                ...
                Text('Promedio Verano:', style: TextStyle(fontSize: 15, color: Colors.grey[700])),
                Text(
                  '${data['avg_verano'].toStringAsFixed(2)} USD/MWh',
                  style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.blue),
                ),
                const SizedBox(height: 24),
                Text('Promedio Histórico Global:', style: TextStyle(fontSize: 15, color: Colors.grey[700])),
                Text(
  ...
  Widget _buildHistorialTab() {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: MockDatabase.getHistorial(idBarra),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          return const Center(child: Text('No hay registros históricos para esta barra.'));
        }
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            return ListTile(
              ...
              subtitle: Text('Hora: ${registro['hora']} | Temporada: ${registro['temporada']}'),
              trailing: Text(
